...
rpm_t rpm_from_time_diff(timeval_t t1, degrees_t deg) {
  timeval_t ticks_per_degree = t1 / deg;
  rpm_t rpm  = (TICKRATE / 6) / ticks_per_degree;
  return rpm;
}

timeval_t time_from_rpm_diff(rpm_t rpm, degrees_t deg) {
